...
detected_in_human = sum([face_detector(file) for file in human_files_short])
detected_in_dog = sum([face_detector(file) for file in dog_files_short])
print('human_files 的前100张图像中，能够检测到人脸的图像占{}%.'.format(detected_in_human))
print('dog_files 的前100张图像中，能够检测到人脸的图像占{}%.'.format(detected_in_dog))
...
# __回答:__
# 
# 我觉得不合理。实际应用场景可能多变，光线和图像采集设备需要进行专业的调整，这对一般用户而言可能存在难度。如果没有清晰的面部特征，至少可辨别面部及五官的轮廓特征。如果一张不够清晰的照片提供的信息不够，可以要求用户提供多张同一目标的图形进行图像合成，进而获取增强后的图像。
...
dogs_in_humans = sum([dog_detector(file) for file in human_files_short])
dogs_in_dogs = sum([dog_detector(file) for file in dog_files_short])
print('human_files_short中图像检测到狗狗占{}%。'.format(dogs_in_humans))
print('dog_files_short中图像检测到狗狗占{}%。'.format(dogs_in_dogs))
...
# 
# 根据上图搭建了CNN模型。
# 此模型由3个连续的卷积层-池化层结构组成，卷积层的过滤器个数是16-32-64递增，从前到后卷积层提取的输入特征越来越多，通过反向传播不断优化权重参数。卷积层后面的池化层降低了数据的空间维度，有利于减少出现过拟合的情况。
...
### TODO: 定义你的网络架构
model.add(Conv2D(16, 2, strides = 1, padding='valid',input_shape=(224,224,3), activation='relu'))
model.add(MaxPooling2D((2, 2), strides=None, padding='valid'))
model.add(Conv2D(32, 2, strides = 1, padding='valid',activation='relu'))
model.add(MaxPooling2D((2, 2), strides=None, padding='valid'))    
model.add(Conv2D(64, 2, strides = 1, padding='valid',activation='relu'))
model.add(MaxPooling2D((2, 2), strides=None, padding='valid'))   
...
his = model.fit(train_tensors, train_targets, 
...
vgg_fit = VGG16_model.fit(train_VGG16, train_targets, 
...
# summarize history for accuracy
plt.plot(vgg_fit.history['acc'])
plt.plot(vgg_fit.history['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.show()
# summarize history for loss
plt.plot(vgg_fit.history['loss'])
plt.plot(vgg_fit.history['val_loss'])
plt.title('model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
...
# __回答:__ 
# 
# 模型使用一个ResNet50预训练模型，在此预训练模型的输出添加全局平均池化层，使用带有softmax激活函数的全连接层作为最终输出层。
# 
# ResNet是一个50层的CNN网络，并且使用残差网络来避免层次过深后出现训练精度下降的问题。使用残差网络，在反向传递过程中，较浅层的网络参数也能得到有效更新。此前使用的VGG16预训练模型则没有使用残差网络。
# ResNet在ImageNet数据集训练和测试中取得非常好的结果，这里我们加载了Keras中提供的50层ResNet的预训练模型的权重数据，在此基础上进行全局平均池化和softmax全链层输出。
# 
# 使用该预训练模型，我们可以直接获取模型的训练数据，并不需为此付出计算和时间资源。
...
resnet_fit = Resnet50_model.fit(train_Resnet50, train_targets, 
...
# summarize history for accuracy
plt.plot(resnet_fit.history['acc'])
plt.plot(resnet_fit.history['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
# 从模型训练过程中精度和损失曲线可以看到,随着模型训练周期的增加，模型并没有取得更好的验证结果，甚至出现了过拟合。
...
predict_dog_breed('/data/dog_images/test/004.Akita/Akita_00244.jpg')
...
def final_detector(img_path):
    ...
    if face_detector(img_path):
        msg = "It's a hunman. It looks like a {}.".format(dog_breed)
    elif dog_detector(img_path):
        msg = "It's a dog. Specificly it's a {}.".format(dog_breed)
    else:
        msg = "Error, nothing detected."
    #显示输入路径的图片
    io.imshow(img_path)
    return msg    
...
img_path = '/data/dog_images/test/004.Akita/Akita_00244.jpg'
final_detector(img_path)
...
# 载入测试照片的路径列表
img_paths = sorted(np.array(glob('FinalTest/**.jpg')))

final_detector(img_paths[0])


# In[40]:


final_detector(img_paths[1])


# In[41]:


final_detector(img_paths[2])


# In[42]:


final_detector(img_paths[3])


# In[43]:


final_detector(img_paths[4])


# In[44]:


final_detector(img_paths[5])


# 模型对这六张图片的预测结果和我的预期基本一致，在测试集中我们得到了超过80%的预测准确率，这6张图片中两只狗都预测正确，两张人的照片也都识别成功。
# 
# 进一步改进模型的方法：
# 
# 1.在训练模型后面继续增加卷积网络，或者使用更深的ResNet152网络；
# 
# 2.使用图像增强来扩充训练集；
# 
# 3.此处使用的是ResNet-50架构的迁移学习，可以使用其他预训练模型进行迁移学习，并与此处的ResNet-50模型进行比较。
